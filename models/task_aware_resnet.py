...
class TaskAwareResNet(nn.Module):
    def __init__(self, block, layers, num_tasks=1, num_classes_per_task=100):
        super(TaskAwareResNet, self).__init__()
        self.in_channels = 64
...
def task_aware_resnet(depth, num_tasks=1, num_classes_per_task=100):
    """
    Creates a task-aware ResNet model with separate classification heads for each task.
    ...
    Returns:
    nn.Module: The task-aware ResNet model.
    """
    depths_to_layers = {
    ...
    return TaskAwareResNet(block, layers, num_tasks=num_tasks, num_classes_per_task=num_classes_per_task)