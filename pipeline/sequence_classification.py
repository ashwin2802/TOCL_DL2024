...
def generate_task_datasets():
    # Split the dataset into tasks based on the domain
    tasks = {}
    for domain in set(dataset['train']['domain']):
        tasks[domain] = dataset.filter(lambda x: x['domain'] == domain)

    # Convert to DatasetDict for easy handling
    task_datasets = {domain: DatasetDict({
        "train": task.filter(lambda x: x['domain'] == domain),
        "test": dataset['test'].filter(lambda x: x['domain'] == domain)
