...
def main(args): 
    ...
    for metric_name, values in ci_results.items():
        if metric_name == "intermediate_representations":
            pass
            # # Number of tasks and hidden dimension
            # num_tasks = len(values[0])  # Number of tasks (from the first iteration's list)
            # hidden_dim = values[0][0].shape[1]  # Hidden representation size

            # print(f"Processing '{metric_name}' with {len(values)} ci_iterations, {num_tasks} tasks, and hidden_dim = {hidden_dim}", flush=True)

            # # Initialize containers for mean and std
            # mean_representations = []
            # std_representations = []

            # # Loop over task indices
            # for task_id in range(num_tasks):
            #     # Collect all tensors for the current task across all iterations
            #     task_tensors = [iteration[task_id] for iteration in values]  # List of tensors, shape [ci_iterations, num_tasks, hidden_dim]
            #     print(f"Task {task_id}: Collected {len(task_tensors)} tensors for ci_iterations.", flush=True)

            #     # Stack into a single tensor: [ci_iterations, num_tasks, hidden_dim]
            #     stacked_task_tensors = torch.stack(task_tensors)  # Shape: [ci_iterations, num_tasks, hidden_dim]
            #     print(f"Task {task_id}: Stacked tensor shape: {stacked_task_tensors.shape}", flush=True)

            #     if stacked_task_tensors.size(0) == 1:  # Single iteration case
            #         # Mean is the tensor itself
            #         task_mean = stacked_task_tensors.squeeze(0)  # Shape: [num_tasks, hidden_dim]
            #         print(f"Task {task_id}: Single iteration detected. Mean shape: {task_mean.shape}", flush=True)
            #         # Std is explicitly set to zeros
            #         task_std = torch.zeros_like(task_mean)  # Shape: [num_tasks, hidden_dim]
            #         print(f"Task {task_id}: Single iteration detected. Std shape: {task_std.shape}", flush=True)
            #     else:
            #         # Compute mean and std along the first dimension (ci_iterations)
            #         task_mean = torch.mean(stacked_task_tensors, dim=0)  # Shape: [num_tasks, hidden_dim]
            #         task_std = torch.std(stacked_task_tensors, dim=0)    # Shape: [num_tasks, hidden_dim]
            #         print(f"Task {task_id}: Mean shape: {task_mean.shape}, Std shape: {task_std.shape}", flush=True)

            #     # Append the mean and std tensors for the task
            #     mean_representations.append(task_mean)
            #     std_representations.append(task_std)
            #     print(f"Task {task_id}: Mean and std appended.", flush=True)

            # # Save results in the desired format: list of `num_tasks` tensors
            # # Each tensor in the list has shape [num_tasks, hidden_representation]
            # metrics_stats[f"{metric_name}_mean"] = mean_representations
            # metrics_stats[f"{metric_name}_std"] = std_representations
            # print(f"Finished processing '{metric_name}'. Saved mean and std.", flush=True)
        else:
