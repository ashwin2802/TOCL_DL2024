...
def hierarchical_clustering(similarity_matrix, mode):
    ...
    while similarity_matrix.shape[0] > 1:
        ...
        if mode == 'max':
            ...
            solution = maximize_pairwise_similarity(similarity_matrix)
        else: 
            # Solve for the current similarity matrix
...
if __name__ == "__main__":
    ...
    path_to_opt_grouping = "./similarity_matrices/hierarchical_{}_partition_CIFAR-100_cambdridge_similarity_with_grad_prod_task_aware_resnet-18-2-50_epochs_10.json"
    """
    path_to_similarity_matrix = "path/to/similarity/matrix.json"
    path_to_opt_grouping = "path/to/output_{}_grouping.json"    

    # Load similarity matrix
    with open(path_to_similarity_matrix, 'r') as f:
        similarity_matrix = json.load(f)

    # Convert JSON list of lists to a NumPy array and consider the first 50 classes
    similarity_matrix = np.array(similarity_matrix)[:50, :50]

    # Perform hierarchical clustering for each mode
    for mode in ['min', 'max']:
        print(f"Performing hierarchical clustering for mode: {mode}")

        # Perform clustering
        leaf_clusters, hierarchy = hierarchical_clustering(similarity_matrix, mode)
        ...
        output_path = path_to_opt_grouping.format(mode)
